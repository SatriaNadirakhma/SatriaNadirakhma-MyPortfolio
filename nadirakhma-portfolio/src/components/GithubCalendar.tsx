...
export const GithubCalendar = memo(function GithubCalendar({
  ...
  // Arrow affordance: only meaningful once the calendar is actually wider
  // than its box (narrow viewport, or a long history range). Re-checked on
  // resize/scroll/content change so it disappears again the moment the
  // grid fits without scrolling — no dead controls sitting around.
  ...
  }, [weeks, gameActive, fetchedData, dataProp]);
  ...
  return (
    ...
    >
      <div className="w-full flex flex-col gap-3 p-3">
        ...
        {/* Scroll arrows — only exist while the grid actually overflows its
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-x-4">
          {/* legend (left) */}
          {showLegend && (
          ...
          {/* stats line (right) */}
          {showStats && (
            <div className="flex flex-wrap justify-start sm:justify-end text-sm font-sans tracking-wide">
              <a
