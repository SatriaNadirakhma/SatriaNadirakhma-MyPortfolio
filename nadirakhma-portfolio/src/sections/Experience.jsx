...
import { NavArrowRight as ChevronRight, ArrowRight } from "iconoir-react";
...
/**
 * Experience as spaced cards: one bordered entry per position, inset
 * from the section edges with gaps between, each rising in with a
 * staggered scroll reveal and lifting slightly on hover. The emerald
 * mark orients this section (same signal across the site). Dates in
 * tabular figures so the timeline reads like a column.
 */
const Experience = () => {
    ...
    <section id={SECTION_IDS.experience} className="px-5 sm:px-8 ">
      <Reveal>
        <div className="relative max-w-7xl mx-auto border-x border-gray-300 dark:border-white/[0.14] p-6 sm:p-8 lg:p-12 bg-[#fafafa]/80 dark:bg-[#080808]/80 backdrop-blur-md">
          <SectionHeader
            title="My journey."
            description="Five chapters across engineering, journalism, and design — each one informing the next."
          />
          ...
          <div className="mt-8 sm:mt-10 flex flex-col gap-3 sm:gap-4">
            {ROWS.map((row, i) => (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: reduceMotion ? 0 : i * 0.08, ease: "easeOut" }}
                whileHover={reduceMotion ? undefined : { y: -3 }}
                className="rounded-[4px] border border-[#e5edf5] dark:border-white/[0.07] bg-white dark:bg-white/[0.02] px-6 sm:px-8 py-8 sm:py-10 transition-colors duration-150 hover:border-gray-300 dark:hover:border-white/15"
              >
                <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-1.5 flex items-center gap-2 text-sm font-normal text-gray-500 dark:text-white/40">
                      <span
                        aria-hidden="true"
                        className="w-1.5 h-1.5 rounded-[1px] bg-emerald-500/70 dark:bg-emerald-400/70 shrink-0"
                      />
                      {row.company}
                    </p>
